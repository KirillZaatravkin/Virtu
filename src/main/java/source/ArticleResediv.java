...
public class ArticleResediv {
    ...
    public List<ArticleStat> FilterStat(String article, java.sql.Date d1, java.sql.Date d2, String sorts, String interval, String cact, String regionMask) {
        ...
        ResultSet rs = null;
        try {
            if (article.equals("5.35.1") || article.equals("7.27") || article.equals("14.16")) {
                if (article.equals("5.35.1")) {
                    statement = connection.prepareStatement("select   cact, MAX(facktaddr) as facktaddr, count(id) as kol, lastname, firstname, birthday, middlename,  max(datep) as md , max(datep) - INTERVAL '365 day' as md2 from st_5_35_1 where  facktaddr LIKE ? and datep BETWEEN  ? and ?  and cact LIKE ? group by lastname, firstname,cact, middlename, birthday, article    order by kol DESC ;");

                } else if (article.equals("7.27")) {
                    statement = connection.prepareStatement("select   cact, MAX(facktaddr) as facktaddr, count(id) as kol, lastname, firstname, birthday, middlename, article, max(datep) as md , max(datep) - INTERVAL '365 day' as md2 from st_7_27  where facktaddr LIKE ? and datep BETWEEN  ? and ? and cact LIKE ? group by lastname, firstname,cact, middlename, birthday, article    order by kol DESC ;");

                } else if (article.equals("14.16")) {
                    statement = connection.prepareStatement("select   cact, MAX(facktaddr) as facktaddr, count(id) as kol, lastname, firstname, birthday, middlename, article, max(datep) as md , max(datep) - INTERVAL '365 day' as md2 from st_14_16 where facktaddr LIKE ? and datep BETWEEN  ? and ?  and cact LIKE ? group by lastname, firstname,cact, middlename, birthday, article    order by kol DESC ;");
                }

                statement.setString(1, regionMask);
                statement.setDate(2, d1);
                statement.setDate(3, d2);
                statement.setString(4, cact);

            } else if (article.equals("12.8_12.6")) {
                statement = connection.prepareStatement("select    MAX(facktaddr) as facktaddr, count(id) as kol, lastname, firstname, birthday, middlename, article, max(datep) as md , max(datep) - INTERVAL '365 day' as md2 from st_12_8_st_12_6 where facktaddr LIKE ? and datep BETWEEN  ? and ?  group by lastname, firstname,middlename, birthday, article    order by kol DESC ;");
            ...
            while (rs.next()) {
                ...
                if (article.equals("5.35.1") || article.equals("7.27") || article.equals("14.16")) {
                    articleStat.setCact(rs.getString("cact"));
                }
        ...
        try {
            if (article.equals("5.35.1")) {
                statement2 = connection.prepareStatement("select count(id) as kol, lastname, firstname, birthday, middlename, article, cact  from st_5_35_1 WHERE  datep BETWEEN ? and ? and lastname=? and firstname=? and middlename=? and birthday=? and cact=?group by cact, lastname, firstname, middlename, birthday, article HAVING count(id)>= 2 order by kol dESC ;");
            } else if (article.equals("7.27")) {
                statement2 = connection.prepareStatement("select count(id) as kol, lastname, firstname, birthday, middlename, article, cact  from st_7_27 WHERE  datep BETWEEN ? and ? and lastname=? and firstname=? and middlename=? and birthday=?  and cact=?group by cact, lastname, firstname, middlename, birthday, article HAVING count(id)>= 2 order by kol dESC ;");
            } else if (article.equals("14.16")) {
                statement2 = connection.prepareStatement("select count(id) as kol, lastname, firstname, birthday, middlename, article, cact  from st_14_16 WHERE  datep BETWEEN ? and ? and lastname=? and firstname=? and middlename=? and birthday=? and cact=? group by cact, lastname, firstname, middlename, birthday, article HAVING count(id)>= 2 order by kol dESC ;");
            } else {
                statement2 = connection.prepareStatement("select count(id) as kol, lastname, firstname, birthday, middlename, article from st_12_8_st_12_6 WHERE  datep BETWEEN ? and ? and lastname=? and firstname=? and middlename=? and birthday=? group by  lastname, firstname, middlename, birthday, article HAVING count(id)>= 2 order by kol dESC ;");
            }
            for (int i = 0; i < stat.size(); i++) {
                ...
                if (article.equals("5.35.1") || article.equals("7.27") || article.equals("14.16")) {
                    statement2.setString(7, stat.get(i).getCact());
                }
                ...
                while (rs2.next()) {
                    ...
                    if (article.equals("5.35.1") || article.equals("7.27") || article.equals("14.16")) {
                        arStat.setCact(rs2.getString("cact"));
                    }
